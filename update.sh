...
function reinstall_bot() {
  ...
  case "$response" in
    [yY][eE][sS]|[yY])
      ...
      bash -c "$(curl -Lfo- https://raw.githubusercontent.com/aliakbarnajmi/Hiddify-Telegram-Bot/refs/heads/main/install.sh)"
...
#     python3 /Hiddify-Telegram-Bot/update.py --update-v4-v5
...
function get_backup() {
  ...
  if cp /Hiddify-Telegram-Bot/Database/hidyBot.db /Hiddify-Telegram-Bot/Database/hidyBot.db.bak; then
...
if [ ! -f /Hiddify-Telegram-Bot/version.py ]; then
  reinstall_bot
else

  current_version=$(python3 /Hiddify-Telegram-Bot/version.py --version)  
  get_backup
  update_bot
  ...
  if python3 /Hiddify-Telegram-Bot/update.py --current-version "$current_version" --target-version "$target_version"; then
      echo "update.py has been run."
  else
