...
class UserDBManager:
    ...
    def add_server(self, url, user_limit, title=None, description=None, status=True, default_server=False):
        cur = self.conn.cursor()
        try:
            admin_path = url.split("/")[3]
            admin_uuid = url.split("/")[4]
            base_url = "https://" + urlsplit(url).netloc + admin_path
            user_path = api.admin_all_config(base_url, admin_uuid)
            cur.execute(
                "INSERT INTO servers(url,title,description,user_limit,status,base_url,admin_path,admin_uuid,user_path,default_server) VALUES(?,?,?,?,?,?,?,?,?,?)",
                (url, title, description, user_limit, status,base_url, admin_path, admin_uuid, user_path, default_server))
            self.conn.commit()
